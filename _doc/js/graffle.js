Raphael.fn.connection = function (obj1, obj2, line, bg, lineoffsety)
{
    if(!lineoffsety)
    {
        lineoffsety = (obj1.lineoffsety) ? (obj1.lineoffsety) : 0;
    }
...
graffle.setup = function (r)
{
    ...
    for (var i = 1/*0 = r*/, ii = sets.length; i < ii; i++)
    {
        ...
        var c = r.connection(sets[i].from, sets[i].to, sets[i].linecolor, undefined, sets[i].lineoffsety);
        connections.push(c);
        if(!document.all && sets[i].ani)
        {
	        ...
	        var anim = Raphael.animation({ progress: 1 }, 2000, 'easeOut', function() {
                if(document.all)this.hide(); }
            ).delay(1000*i).repeat(document.all ? 0 : Infinity);
	        var set = r.setFinish();
